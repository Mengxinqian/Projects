...
# Name: Mengxin Qian (A)        Yanli Qian (B)
# Email: 11mq4@berkeley.edu
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        position = self.place
        available = 0
        while position != hive and position != []:
            if available >= self.min_range and available <= self.max_range:
                if position.bees != []:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            beeinplace = self.place.bees[:]
            for bee in beeinplace:
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        if not self.real_queen:
            ...
            if self.place.ant != self and (not self.place.ant in QueenAnt.doubled_ants):       # bodyguard present
                self.place.ant.damage *= 2
                QueenAnt.doubled_ants += [self.place.ant]

            tmp_place = self.place.entrance
            while tmp_place:
                if tmp_place.ant and tmp_place.ant.container:
                    if (not (tmp_place.ant.ant in QueenAnt.doubled_ants)) and tmp_place.ant.ant:
                        tmp_place.ant.ant.damage *= 2
                        QueenAnt.doubled_ants += [tmp_place.ant.ant]
                if (not (tmp_place.ant in QueenAnt.doubled_ants)) and tmp_place.ant:
                    tmp_place.ant.damage *= 2
                    QueenAnt.doubled_ants += [tmp_place.ant]
