...
def extract_words(text):
    ...
    for i in range(len(text_list)):
        if not (text_list[i] in ascii_letters):
            text_list[i] = ' '
    text = "".join(text_list)
...
def analyze_tweet_sentiment(tweet):
    ...
    while i < len_list:
        if has_sentiment(get_word_sentiment(word_list[i])):
            sum_list += sentiment_value(get_word_sentiment(word_list[i]))
            n += 1
        i += 1
    if n == 0:
...
def find_centroid(polygon):
    ...
    for i in range(len(polygon)-1):
        ...
        xi_1 = latitude(polygon[i+1])
        yi_1 = longitude(polygon[i+1])
        Cx += 1/(6 * A) * (xi + xi_1) * (xi * yi_1 - xi_1 * yi)
        Cy += 1/(6 * A) * (yi + yi_1) * (xi * yi_1 - xi_1 * yi)
...
def find_state_center(polygons):
    ...
    """
    "*** YOUR CODE HERE ***"
    total_centroid_latitude = 0
    total_centroid_longitude = 0
    total_area = 0
    for polygon in polygons:
        centroid = find_centroid(polygon)
        total_centroid_latitude += centroid[0]*centroid[2]
        total_centroid_longitude += centroid[1]*centroid[2]
        total_area += centroid[2]
    total_centroid_latitude = total_centroid_latitude/total_area
    total_centroid_longitude = total_centroid_longitude/total_area
    return make_position(total_centroid_latitude, total_centroid_longitude)
...
def draw_centered_map(center_state='TX', n=10):
    ...
    #  Mark the center state with a red dot
    wait()
